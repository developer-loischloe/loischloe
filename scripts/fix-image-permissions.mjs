import { Client, Storage, Databases, Query, Permission, Role } from "node-appwrite";

const client = new Client();
client.setEndpoint("https://cloud.appwrite.io/v1")
  .setProject("65ed75e73895ca457661")
  .setKey("");

const storage = new Storage(client);
const db = new Databases(client);
const BUCKET_ID = "65eda0fc4b0db5feb54e";
const DB_ID = "65ed81d8651cfe000e96";
const PRODUCT_COL = "65ed81e9258fd5b3a7b8";
const ADMIN_USER = "65ed755d93f357c45240";

async function main() {
  const slugs = ["glam-on-the-go", "hydra-lip-duo"];

  for (const slug of slugs) {
    const result = await db.listDocuments(DB_ID, PRODUCT_COL, [Query.equal("slug", slug)]);
    const product = result.documents[0];
    console.log(`\n${slug} - ${product.images?.length || 0} images`);

    for (const img of product.images || []) {
      console.log(`  Fixing file: ${img.image_id}`);
      await storage.updateFile(BUCKET_ID, img.image_id, undefined, [
        Permission.read(Role.user(ADMIN_USER)),
        Permission.update(Role.user(ADMIN_USER)),
        Permission.delete(Role.user(ADMIN_USER)),
      ]);
      console.log(`  Permissions set!`);
    }
  }
  console.log("\nAll file permissions fixed!");
}

main().catch(console.error);
